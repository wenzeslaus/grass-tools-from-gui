...
class GCPPanel(MapPanel, ColumnSorterMixin):
    ...
    def __init__(
        ...
        self.GCPcount = 0
        # Start with one inactive point per fiducial mark: the image
        # coordinates are all zeros and the target coordinates come from
        # the camera file.
        image_coordinates = []
        target_coordinates = []
        for index in range(numberOfFiducial):
            image_coordinates.extend(("0", "0"))
            target_coordinates.extend((dataFiducialX[index], dataFiducialY[index]))
        ...
        if ret == 0 and image_coordinates:
            ...
            if ret == 0:
                ret = RunCommand(
                    ...
                    operation="disable",
                    points=",".join(
                        str(number) for number in range(1, numberOfFiducial + 1)
                    ),
                )
